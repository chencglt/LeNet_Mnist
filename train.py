...
def inference(input_x, input_y):
    ...
    le_network = LeNet()
    y = le_network.network_fn(x)
    #y_test = LeNet(x_t, True)
...
def main_loop():
    ...
    with tf.Session() as sess:
        x = tf.placeholder(tf.float32, [None, cfg.IMAGE_W, cfg.IMAGE_W, 1], name='x-input')
        y_ = tf.placeholder(tf.float32, [None, cfg.LABEL_SIZE], name='y-input')
